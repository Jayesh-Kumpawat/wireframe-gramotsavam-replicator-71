      ...
      levels: [
        {
          ...
          matches: [
            { id: 1, teamA: "Team A", teamB: "Team B", status: "Completed", score: "25 - 23", venue: "Coimbatore" },
            { id: 2, teamA: "Team A", teamB: "Team C", status: "Upcoming", date: "3/01/25", venue: "IYC" }
          ]
        },
        {
          name: "Division Level",
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="p-4">
          <div className="space-y-4">
            {selectedMatch.level.matches.map((match) => (
              <div key={match.id} className="border-2 border-foreground rounded-lg p-4">
                ...
                {match.date && (
                  <div className="text-sm text-foreground/70 text-center mt-1">
      ...
      <div className="p-4">
        ...
        <div className="space-y-6">
          {tournaments.map((tournament) => (
            <div key={tournament.sport}>
              <h2 className="font-medium mb-3">{tournament.sport}</h2>
              
              {tournament.levels.length > 0 ? (
                <div className="space-y-3">
                  {tournament.levels.map((level, index) => (
                    <div key={index} className="border-2 border-foreground rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <div className="font-medium text-sm">{level.name}</div>
                          <div className="text-xs text-foreground/70">
                            {level.status === "Completed" ? `Completed on ${level.date}` : 
                             level.status === "Upcoming" ? `scheduled on ${level.date}` : 
                             level.date}
                          </div>
                        </div>
                ...
                <div className="border-2 border-foreground rounded-lg p-4 text-center text-foreground/50">
